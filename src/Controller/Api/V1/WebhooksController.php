...
class WebhooksController extends AppController
{
    ...
    public function stripe()
    {
        ...
        switch ($event->type) {
            case 'checkout.session.completed':
                ...
                if (isset($session->customer) && isset($session->subscription)) {
                    ...
                    Log::debug(
                        'Checkout Webhook: Searching for customer ' . $session->customer
                        . '. Found: ' . ($user ? 'Yes (ID:' . $user->id . ')' : 'No'),
                        ['payments'],
                    );
                    ...
                    if ($user) {
                        ...
                        Log::debug(
                            'Checkout Webhook: Table columns: '
                            . json_encode($subscriptionsTable->getSchema()->columns()),
                            ['payments'],
                        );
                        ...
                        if (!$subscription) {
                            ...
                            $subscription->id = Text::uuid();
                            $subscription->user_id = $user->id;
                            Log::debug(
                                'Checkout Webhook: Created new subscription entity with ID: ' . $subscription->id,
                                ['payments'],
                            );
                        }
                        ...
                        try {
                            ...
                            $subscription->max_devices_allowed = Configure::read(
                                "Subscriptions.{$subscription->plan}.max_devices_allowed",
                            );

                            Log::debug(
                                'Checkout Webhook: Subscription object keys: '
                                . json_encode(array_keys($stripeSubscription->toArray())),
                                ['payments'],
                            );
                            ...
                            if ($start) {
                                $subscription->current_period_start = DateTime::createFromTimestamp((int)$start);
                                Log::debug(
                                    'Checkout Webhook: Setting Start: '
                                    . $subscription->current_period_start->format('Y-m-d H:i:s'),
                                    ['payments'],
                                );
                            }
                            if ($end) {
                                $subscription->current_period_end = DateTime::createFromTimestamp((int)$end);
                                Log::debug(
                                    'Checkout Webhook: Setting End: '
                            ...
                            Log::debug(
                                'Checkout Webhook: Dirty fields after dates: '
                                . json_encode($subscription->getDirty()),
                                ['payments'],
                            );

                            if ($subscriptionsTable->save($subscription)) {
                                Log::info(
                                    'Checkout Webhook: Subscription successfully updated for user ' . $user->id,
                                    ['payments'],
                                );
                                // Reload to check actual DB values
                                $checkSub = $subscriptionsTable->get($subscription->id);
                                $startStr = $checkSub->current_period_start
                                    ? $checkSub->current_period_start->format('Y-m-d H:i:s') : 'NULL';
                                $endStr = $checkSub->current_period_end
                                    ? $checkSub->current_period_end->format('Y-m-d H:i:s') : 'NULL';
                                Log::debug(
                                    'Checkout Webhook: Verified DB Values - Start: ' . $startStr
                                    . ', End: ' . $endStr,
                                    ['payments'],
                                );
                            } else {
                                Log::error(
                                    'Checkout Webhook: Failed to save subscription. Errors: '
                                    . json_encode($subscription->getErrors()),
                                    ['payments'],
                                );
                            }
                            ...
                            Log::error(
                                'Failed to retrieve subscription during checkout.session.completed: '
                                . $e->getMessage(),
                                ['payments'],
                            );
                        }
            ...
            case 'customer.subscription.updated':
                ...
                if ($subscription) {
                    ...
                    if ($subscriptionsTable->save($subscription)) {
                        Log::info(
                            'Subscription Updated Webhook: Success for ' . $stripeSubscription->id,
                            ['payments'],
                        );
                    } else {
                        Log::error(
                            'Subscription Updated Webhook: Save Failed. Errors: '
                            . json_encode($subscription->getErrors()),
                            ['payments'],
                        );
                    }
                ...
                if (isset($invoice->subscription)) {
                    ...
                    if ($subscription) {
                        try {
                            ...
                            if ($subscriptionsTable->save($subscription)) {
                                Log::info(
                                    'Invoice Paid Webhook: Subscription dates updated: ' . $invoice->subscription,
                                    ['payments'],
                                );
                            } else {
                                Log::error(
                                    'Invoice Paid Webhook: Save Failed. Errors: '
                                    . json_encode($subscription->getErrors()),
                                    ['payments'],
                                );
                            }
                            ...
                            Log::error(
                                'Failed to update subscription dates on invoice.payment_succeeded: '
                                . $e->getMessage(),
                                ['payments'],
                            );
                        }
        ...
        return $this->response->withType('application/json')
            ->withStringBody((string)json_encode(['received' => true]));
    }
}
